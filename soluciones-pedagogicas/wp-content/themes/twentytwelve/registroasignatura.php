...
if(isset($_POST['btn_registro_asignatura'])){
	...
	{	$asignaturas_del_curso = array();
		$asignaturas_del_curso[0]  = get_post_meta($curso, 'Asignaturas', true);
		array_push($asignaturas_del_curso, $id_asignatura_creada);		
		update_post_meta($curso, 'Asignaturas', $asignaturas_del_curso  );
		//update_post_meta($curso, 'Asignaturas', $id_asignatura_creada  );
...
<tr>
	...
	?>
		<select multiple name="select_curso[]" size="<? echo $cantidad_asignaturas; ?>">     
    <?php
	echo '<option></option>'; 	 

...
function wp_login_form( $args = array() ) {
	...
	$form = '		
		<form class="form-horizontal" role="form" name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
			<div class="form-group">				
					<label class="col-lg-4 letras_blanca_normal" for="' . esc_attr( $args['id_username'] ) . '">' . esc_html( $args['label_username'] ) . '</label>					
				<div class="col-lg-6">
					<input type="text" name="log" id="' . esc_attr( $args['id_username'] ) . '" class="input form-control input_con_sombra" value="' . esc_attr( $args['value_username'] ) . '" size="20" />
				</div>					
			</div>
			
			<div class="form-group">							
					<label  class="col-lg-4 letras_blanca_normal" for="' . esc_attr( $args['id_password'] ) . '">' . esc_html( $args['label_password'] ) . '</label>
				
				<div class="col-lg-6">	
					<input type="password" name="pwd" id="' . esc_attr( $args['id_password'] ) . '" class="input form-control input_con_sombra" value="" size="20" />	
				</div>	
			</div>			
			
			<div class="form-group">				
			' . $login_form_middle . '
			' . ( $args['remember'] ? '<label class="col-lg-4 letras_blanca_normal"><input class="input_con_sombra" name="rememberme" type="checkbox" id="' . esc_attr( $args['id_remember'] ) . '" value="forever"' . ( $args['value_remember'] ? ' checked="checked"' : '' ) . ' /> ' . esc_html( $args['label_remember'] ) . '</label>' : '' ) . '
				<div class="col-lg-8">
					<button class="boton" type="submit" name="wp-submit" id="' . esc_attr( $args['id_submit'] ) . '"/>'. esc_attr( $args['label_log_in'] ) .'</button>					
					<input type="hidden" name="redirect_to" value="' . esc_url( $args['redirect'] ) . '" />
